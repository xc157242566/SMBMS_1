...
$(function(){
	$(".viewBill").on("click",function(){
		...
		$.ajax({
			type:"GET",
			url:path+"/jsp/billinfo",
			data:{billid:obj.attr("billid")},
			dataType:"json",
			success:function(data){
				$("#billinfo #billCode").val(data.billCode)
				$("#billinfo #productName").val(data.productName)
				$("#billinfo #productUnit").val(data.productUnit)
				$("#billinfo #productCount").val(data.productCount)
				$("#billinfo #totalPrice").val(data.totalPrice)
				$("#billinfo #providerName").val(data.providerName)
			},
			error:function(data){
				alert("对不起，查看失败");
			}
